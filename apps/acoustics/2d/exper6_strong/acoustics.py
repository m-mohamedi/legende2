...
def acoustics2D(mxp = 400,finalt=1,iplot=False,htmlplot=False,use_petsc=False,outdir='./_output',solver_type='classic'):
    """
...
if __name__=="__main__":
    ...
    size_16 = 16
    tb1call=time()        
    acoustics2D(mxp = mxp,finalt=(0.2/np.sqrt(size_16))/40,use_petsc=True,outdir='exper6_b/_output_p1')
    PETSc.COMM_WORLD.barrier()
    
    tb2call=time()
    rank =PETSc.Comm.getRank(PETSc.COMM_WORLD)
    size =PETSc.Comm.getSize(PETSc.COMM_WORLD)    
    acoustics2D(mxp = mxp,finalt=(0.2/np.sqrt(size_16))/40,use_petsc=True,outdir='exper6_b/_output_p2')
    PETSc.COMM_WORLD.barrier()
    tb3call=time()
    if rank in proccessesList:
        funccall = "acoustics2D(mxp = mxp,finalt=0.05/np.sqrt(size_16),use_petsc=True,outdir='exper6_b/_output_p3')"
        cProfile.run(funccall, 'exper6_b/profile'+str(rank)+'_'+str(size))
    else:
        print "process"+str(rank) +"not profiled"
        acoustics2D(mxp = mxp,finalt=0.05/np.sqrt(size_16),use_petsc=True,outdir='exper6_b/_output_p3')
    PETSc.COMM_WORLD.barrier()
    tend = time()
    if rank == 0: 
